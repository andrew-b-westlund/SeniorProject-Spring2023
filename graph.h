...
class Edge {
	...
	vID vid;
	UINT32 weight;
	// padding to round out memory to multiples of 64 bits for speed
	UINT32 pad;
};
...
class vertex {
	...
	void setVertexForward(UINT32 capacity);
	
	// Preconditions: None
	// Postconditions: Backward will be set to have an empty edge list with
	// the passed number of places available
	void setVertexBackward(UINT32 backCapacity);
...
class graph {
	...
	// Preconditions: None.
	// Postconditions: Will set allocation to a positive number up to 16; 
	// affects how much edge/back-edge arrays grow when full
	void setAllocation(INT16 a);
	
	// Preconditions: v.id is less than vNum
	// Postconditions: Empty edge/back-edge arrays of sizes capacity and back-capacity respectively 
	// will be set for v's vertex (reduces linear copying over and affective with addEdge2)
	...
	void createVForward(vID v, UINT32 capacity);
	
	// Sets v's vertex's back-edge array to be empty and have backCapacity empty places
	void createVBackward(vID v, UINT32 backCapacity);
	...
	// Preconditions: vertex(u) and vertex(v) must be existing vertices on the graph.
	// Postconditions: If there is an edge going from vertex(u) to vertex(v), set index to the v's index 
	// in u's neighbor array and return true; otherwise, return false.
	bool isEdge(vID u, vID v, UINT32 &index);
	
	// Preconditions: vertex(u) and vertex(v) must be existing vertices on the graph.
	// Postconditions: If there is a backedge going from vertex(u) to vertex(v), return true; otherwise, return false.
	bool isBackEdge(vID u, vID v);
	
	// Preconditions: vertex(u) and vertex(v) must be existing vertices on the graph.
	// Postconditions: If there is a backedge going from vertex(u) to vertex(v), set index to the v's index 
	// in u's back neighbor array and return true; otherwise, return false.
	bool isBackEdge(vID u, vID v, UINT32 &index);

	// Preconditions: vertex(u) and vertex(v) must be existing vertices; vertices' edge/back-edge arrays must be in order
	// Postconditions: Add an edge from u’s vertex to v’s vertex; if u does not have an edge to v, then an edge with the 
	// passed weight from u to v will be added, and a similar back-edge from v to u will be added. If inc is false and u 
	// already has an edge to v, the weight from u to v will be overwritten by the new weight; if inc is true, then if an 
	//edge already exists, the edge will increase by the passed weight. If the edge already existed, return false; otherwise, return true.
	...
	bool addG2Edge(vID u, vID v, UINT32 weight = 1, bool inc = false);
	...
	// Preconditions: vertex(u) and vertex(v) must be existing vertices
	// Postconditions: An edge from u to v with the passed weight is appended to the end of u's neighbor list
	void addEdge2Forward(vID u, vID v, UINT32 weight = 1);
	
	// Preconditions: vertex(u) and vertex(v) must be existing vertices
	// Postconditions: A back-edge from v to u with the passed weight is appended to the end of v's back-neighbor list
	void addEdge2Backward(vID u, vID v, UINT32 weight = 1);
	...
	// CONSIDER ASKING ABOUT ADDING A setEdge(u, v, w) AND getEdge(u, v) FUNCTION
	...
	// Preconditions: None
	// Postconditions:Print out the ID of each vertex that the indicated vertex points to 
	// and that points to it, as well as all the edges' weights
